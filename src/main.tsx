...
function App(){
 ...
 React.useEffect(()=>{document.title = page === "home" ? "فالسريع | كل اللي محتاجه يوصلك" : `فالسريع | ${{about:"عن فالسريع",services:"خدماتنا",order:"اطلب الآن",support:"الدعم والاستفسارات",complaint:"تقديم شكوى",partner:"الشراكة",join:"انضم إلينا",contact:"تواصل معنا",privacy:"سياسة الخصوصية",terms:"الشروط والأحكام"}[page]}`;},[page]);
 ...
 <main>{page==="home"&&<Home go={go}/>} {page==="about"&&<About/>} {page==="services"&&<Services/>} {page==="order"&&<Order/>} {page==="support"&&<Support/>} {page==="complaint"&&<Complaint/>} {page==="partner"&&<Partner/>} {page==="join"&&<Join/>} {page==="contact"&&<Contact/>} {page==="privacy"&&<Legal title="سياسة الخصوصية" type="privacy"/>} {page==="terms"&&<Legal title="الشروط والأحكام" type="terms"/>}</main>
...
function Support(){
 const [name,setName]=React.useState(""); const [phone,setPhone]=React.useState(""); const [type,setType]=React.useState("دعم فني"); const [message,setMessage]=React.useState("");
 return <WhatsAppForm title="الدعم والاستفسارات" description="لو عندك سؤال أو محتاج مساعدة، اختار نوع التواصل وأرسل رسالتك مباشرة." icon={<Headphones/>} onBuild={()=>name.trim()&&phone.trim()&&message.trim()?["🎧 *دعم / استفسار — فالسريع*","","👤 *الاسم:* "+name.trim(),"📞 *رقم الهاتف:* "+phone.trim(),"📌 *النوع:* "+type,"💬 *الرسالة:* "+message.trim(),"","تم الإرسال من موقع فالسريع."].join("\n"):null}>
  ...
  <label>نوع التواصل<select value={type} onChange={e=>setType(e.target.value)}><option>دعم فني</option><option>استفسار عام</option><option>مشكلة في طلب</option><option>اقتراح</option></select></label>
  <label>اكتب رسالتك<textarea value={message} onChange={e=>setMessage(e.target.value)} rows={5} placeholder="اشرح طلبك أو استفسارك بوضوح..."/></label>
 </WhatsAppForm>
}
...
function Join(){
 ...
 return <WhatsAppForm title="انضم إلى فريقنا" description="لو تريد العمل كمندوب مع فالسريع، أرسل بياناتك الأساسية وسنتواصل معك عند توفر فرصة مناسبة." icon={<Users/>} onBuild={()=>name.trim()&&age.trim()&&phone.trim()&&bike.trim()&&availability.trim()&&shift.trim()&&period.trim()?["🛵 *طلب انضمام للعمل — فالسريع*","","👤 *الاسم:* "+name.trim(),"🎂 *السن:* "+age.trim(),"📞 *رقم الهاتف:* "+phone.trim(),"🏍️ *نوع الموتوسيكل:* "+bike.trim(),"⏰ *الوقت المتاح:* "+availability.trim(),"🌙 *الوردية المفضلة:* "+shift,"📅 *فترة العمل:* "+period,"","تم الإرسال من موقع فالسريع."].join("\n"):null}>
...
function Contact(){const phoneContacts=CONTACTS.filter(x=>x.type==="phone"); const whatsappContacts=CONTACTS.filter(x=>x.type==="whatsapp"); const actions:[Page,string,React.ElementType,string][]=[["order","اطلب الآن",ClipboardList,"ابدأ طلبًا برسالة منظمة"],["support","دعم واستفسارات",Headphones,"مساعدة أو سؤال سريع"],["complaint","تقديم شكوى",TriangleAlert,"أرسل شكواك وتفاصيلها"],["partner","كن شريكًا",Handshake,"مطاعم ومحلات وشركاء"],["join","انضم للعمل",Users,"التقديم للعمل كمندوب"]]; return <PageHero eyebrow="تواصل معنا" title={<>اختر <strong>اللي تحتاجه.</strong></>} text="كل خدمة لها نموذج ورسالة واتساب منفصلة ومنظمة لتسهيل المتابعة."><section className="section contact-section"><h2 className="contact-title">اختر نوع التواصل</h2><div className="grid three action-grid">{actions.map(([p,title,Icon,desc])=><a className="action-card" key={p} href={PAGE_PATHS[p]}><Icon/><div><b>{title}</b><span>{desc}</span></div><ArrowLeft/></a>)}</div><h2 className="contact-title">الاتصال المباشر</h2><div className="grid two">{phoneContacts.map((c,i)=><a className="contact-card" key={i} href={contactHref(c)}><Phone/><div><b>{c.label}</b><span>{c.value}</span><small>اضغط للاتصال مباشرة</small></div></a>)}</div><h2 className="contact-title">واتساب المباشر</h2><div className="grid two">{whatsappContacts.map((c,i)=><a className="contact-card whatsapp" key={i} href={contactHref(c)} target="_blank" rel="noreferrer"><MessageCircle/><div><b>{c.label}</b><span>{c.value}</span><small>اضغط لفتح محادثة واتساب</small></div></a>)}</div></section></PageHero>}
