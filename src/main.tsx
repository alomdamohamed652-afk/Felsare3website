...
import { Phone, MessageCircle, Zap, ShieldCheck, MapPin, Store, Bike, Utensils, Coffee, ShoppingBag, Package, ArrowLeft, Menu, X, Users } from "lucide-react";
import "./styles.css";
...
function App(){
 ...
 <header className="nav"><button className="brand" onClick={()=>go("home")}><span className="brand-mark">F</span><span>فالسريع</span></button><nav>{nav.map(([p,n])=><a className={page===p?"active":""} key={p} href={PAGE_PATHS[p]} onClick={(e)=>{e.preventDefault();go(p);}}>{n}</a>)}</nav><div className="nav-actions"><a className="call-mini" href={PHONE ? "tel:"+PHONE : "#contact"}><Phone size={17}/> اطلب الآن</a><button className="menu" onClick={()=>setMenu(!menu)}>{menu?<X/>:<Menu/>}</button></div></header>
 {menu&&<div className="mobile-nav">{nav.map(([p,n])=><a key={p} href={PAGE_PATHS[p]} onClick={(e)=>{e.preventDefault();go(p);}}>{n}</a>)}</div>}
 <main>{page==="home"&&<Home go={go}/>} {page==="about"&&<About/>} {page==="services"&&<Services/>} {page==="join"&&<Join/>} {page==="contact"&&<Contact/>} {page==="privacy"&&<Legal title="سياسة الخصوصية" type="privacy"/>} {page==="terms"&&<Legal title="الشروط والأحكام" type="terms"/>}</main>
 <footer><div><button className="brand" onClick={()=>go("home")}><span className="brand-mark">F</span><span>فالسريع</span></button><p>اطلبها… توصل فالسريع.</p></div><div className="footer-links">{nav.map(([p,n])=><a key={p} href={PAGE_PATHS[p]} onClick={(e)=>{e.preventDefault();go(p);}}>{n}</a>)}</div><div className="legal-links"><button onClick={()=>go("privacy")}>سياسة الخصوصية</button><span>•</span><button onClick={()=>go("terms")}>الشروط والأحكام</button></div><small>© {new Date().getFullYear()} فالسريع — جميع الحقوق محفوظة</small></footer>
 </div>
}
function Home({go}:{go:(p:Page)=>void}){return <>
<section className="hero"><div className="hero-glow"></div><div className="hero-copy"><span className="eyebrow">قريبًا في مدينتك ⚡</span><h1>كل اللي محتاجه…<strong>يوصلك فالسريع.</strong></h1><p>خدمة توصيل محلية بسيطة وسريعة. حاليًا اطلب بسهولة عبر الهاتف أو واتساب.</p><div className="hero-buttons"><a className="btn primary" href={PHONE ? "tel:"+PHONE : "#contact"}><Phone/> اطلب عبر الهاتف</a><a className="btn secondary" href={WHATSAPP ? contactHref(WHATSAPP) : "#contact"} target={WHATSAPP ? "_blank" : undefined} rel={WHATSAPP ? "noreferrer" : undefined}><MessageCircle/> واتساب</a></div><div className="quick"><span><Zap/> أسرع</span><span><ShieldCheck/> موثوق</span><span><MapPin/> أقرب لك</span></div></div><div className="hero-visual"><div className="speed-lines"></div><img className="hero-image" src="/images/hero-placeholder.svg" alt="صورة توصيل فالسريع" /><div className="image-label">صورة مؤقتة قابلة للاستبدال</div></div></section>
...
<section className="cta"><h2>محتاج حاجة؟</h2><p>اختار الطريقة المناسبة وتواصل معنا مباشرة.</p><div className="hero-buttons center-buttons"><a className="btn primary" href={PHONE ? "tel:"+PHONE : "#contact"}><Phone/> اتصل الآن</a><a className="btn secondary" href={WHATSAPP ? contactHref(WHATSAPP) : "#contact"} target={WHATSAPP ? "_blank" : undefined} rel={WHATSAPP ? "noreferrer" : undefined}><MessageCircle/> واتساب</a></div></section></>}
