...
function App(){
 ...
 React.useEffect(()=>{
   const onPopState=()=>{setPage(pathToPage(window.location.pathname));setChoice(null);setMenu(false);window.scrollTo({top:0,behavior:"auto"});};
   window.addEventListener("popstate",onPopState);
   return ()=>window.removeEventListener("popstate",onPopState);
 },[]);
 const knownPage=page ?? "notFound";
...
function WhatsAppForm({title,description,icon,children,onBuild}:{title:string,description:string,icon:React.ReactNode,children:React.ReactNode,onBuild:()=>string|null}){
 ...
 return <div className="form-page"><section className="section order-section"><div className="order-intro">{icon}<div><span className="eyebrow">تواصل عبر واتساب</span><h2>{title}</h2><p>{description}</p><small>املأ البيانات ثم ستُفتح رسالة منظمة وجاهزة للمراجعة والإرسال.</small></div></div>{sent?<div className="success-card"><span>✓</span><h3>تم تجهيز رسالتك بنجاح</h3><p>تم فتح واتساب بالرسالة الجاهزة. راجع التفاصيل واضغط إرسال داخل واتساب لإتمام التواصل، ثم سنرد عليك في أسرع وقت.</p><button className="btn secondary" onClick={()=>setSent(false)}>إرسال رسالة أخرى</button></div>:<form className="order-form" onSubmit={submit}>{children}{error&&<div className="form-error">{error}</div>}<button className="btn primary order-submit" type="submit"><MessageCircle/> متابعة عبر واتساب</button><p className="privacy-note">🔒 بياناتك لا يتم تخزينها حاليًا داخل الموقع، وتُستخدم فقط لتجهيز رسالة واتساب.</p></form>}</section></div>
}

function Support(){
 const [name,setName]=React.useState(""); const [phone,setPhone]=React.useState(""); const [message,setMessage]=React.useState("");
 return <WhatsAppForm title="الدعم" description="لو تحتاج مساعدة أو لديك مشكلة، أرسل تفاصيلها مباشرة إلى فريق الدعم." icon={<Headphones/>} onBuild={()=>name.trim()&&phone.trim()&&message.trim()?["🎧 *طلب دعم — فالسريع*","","👤 *الاسم:* "+name.trim(),"📞 *رقم الهاتف:* "+phone.trim(),"🛠️ *تفاصيل المشكلة:* "+message.trim(),"","تم الإرسال من موقع فالسريع."].join("\n"):null}><div className="form-grid"><label>الاسم الكامل<input value={name} onChange={e=>setName(e.target.value)} placeholder="اكتب اسمك" autoComplete="name"/></label><label>رقم الهاتف<input value={phone} onChange={e=>setPhone(e.target.value)} inputMode="tel" autoComplete="tel" placeholder="01XXXXXXXXX"/></label></div><label>كيف يمكننا مساعدتك؟<textarea value={message} onChange={e=>setMessage(e.target.value)} rows={5} placeholder="اشرح المشكلة أو ما تحتاجه..."/></label></WhatsAppForm>
}
function Inquiry(){
 const [name,setName]=React.useState(""); const [phone,setPhone]=React.useState(""); const [message,setMessage]=React.useState("");
...
function JoinForm(){
 const [name,setName]=React.useState("");const [age,setAge]=React.useState("");const [phone,setPhone]=React.useState("");const [bike,setBike]=React.useState("");const [area,setArea]=React.useState("");const [availability,setAvailability]=React.useState("");const [shift,setShift]=React.useState("");const [period,setPeriod]=React.useState("");
 return <WhatsAppForm title="انضم كمندوب لفريقنا" description="أكمل البيانات الأساسية بدقة، وسنتواصل معك عند توفر فرصة مناسبة." icon={<Users/>} onBuild={()=>name.trim()&&age.trim()&&phone.trim()&&bike.trim()&&area.trim()&&availability.trim()&&shift&&period?["🛵 *طلب انضمام للعمل — فالسريع*","","👤 *الاسم:* "+name.trim(),"🎂 *السن:* "+age.trim(),"📞 *رقم الهاتف:* "+phone.trim(),"📍 *المنطقة:* "+area.trim(),"🏍️ *نوع الموتوسيكل:* "+bike.trim(),"⏰ *الوقت المتاح:* "+availability.trim(),"🌙 *الوردية المفضلة:* "+shift,"📅 *نظام العمل:* "+period,"","تم الإرسال من موقع فالسريع."].join("\n"):null}>
 <div className="form-grid"><label>الاسم الكامل<input value={name} onChange={e=>setName(e.target.value)} placeholder="اكتب اسمك" autoComplete="name"/></label><label>السن<input value={age} onChange={e=>setAge(e.target.value.replace(/[^0-9]/g,""))} inputMode="numeric" placeholder="مثال: 22"/></label></div>
 <div className="form-grid"><label>رقم الهاتف<input value={phone} onChange={e=>setPhone(e.target.value)} inputMode="tel" autoComplete="tel" placeholder="01XXXXXXXXX"/></label><label>المنطقة<input value={area} onChange={e=>setArea(e.target.value)} placeholder="المدينة / المنطقة"/></label></div>
 <label>نوع الموتوسيكل<input value={bike} onChange={e=>setBike(e.target.value)} placeholder="الماركة / الموديل"/></label>
