...
function App(){
 ...
 {choice&&<ContactChoiceModal title={choice.title} description={choice.description} onClose={()=>setChoice(null)} onPhone={()=>{window.location.href=PHONE_HREF;}} onWhatsApp={()=>{const target=choice.page;setChoice(null);setWhatsappForm(target);}}/>}{whatsappForm&&<WhatsAppModal page={whatsappForm} onClose={()=>setWhatsappForm(null)}/>}
 </div>
}
...
function WhatsAppModal({page,onClose}:{page:Page;onClose:()=>void}){
 const data:any={order:["اطلب الآن",ClipboardList,["الاسم الكامل","رقم الهاتف","تفاصيل الطلب","مكان الاستلام","مكان التوصيل","ملاحظات"]],support:["الدعم",Headphones,["الاسم الكامل","رقم الهاتف","تفاصيل المشكلة"]],inquiry:["استفسار",CircleHelp,["الاسم الكامل","رقم الهاتف","اكتب استفسارك"]],complaint:["تقديم شكوى",TriangleAlert,["الاسم الكامل","رقم الهاتف","تفاصيل الشكوى"]],partner:["تعاون معنا كشريك",Handshake,["اسم النشاط","نوع النشاط","اسم المسؤول","رقم الهاتف","العنوان"]],joinForm:["انضم كمندوب لفريقنا",Users,["الاسم الكامل","السن","رقم الهاتف","نوع الموتوسيكل","الوقت المتاح","الوردية","فترة العمل"]]}; const x=data[page]||data.order; const Icon=x[1]; const [v,setV]=React.useState<string[]>(x[2].map(()=> ""));
 const submit=(e:React.FormEvent)=>{e.preventDefault();if(v.some(a=>!a.trim())){alert("من فضلك املأ جميع البيانات المطلوبة.");return;}const lines=x[2].map((f:string,i:number)=>"• *"+f+":* "+v[i]);sendWhatsApp(["🟠 *"+x[0]+" — فالسريع*","",...lines,"","تم الإرسال من موقع فالسريع."].join("\n"));};
 return <div className="choice-backdrop form-backdrop" onMouseDown={onClose}><div className="choice-modal whatsapp-form-modal" onMouseDown={e=>e.stopPropagation()}><button className="choice-close" onClick={onClose}><X/></button><div className="modal-form-head"><Icon/><div><span className="eyebrow">تواصل عبر واتساب</span><h2>{x[0]}</h2><p>املأ البيانات وسيتم فتح واتساب برسالة منظمة.</p></div></div><form className="order-form modal-form" onSubmit={submit}>{x[2].map((field:string,i:number)=><label key={field}>{field}<input value={v[i]} onChange={e=>setV(a=>a.map((z,j)=>j===i?e.target.value:z))} placeholder={field}/></label>)}<button className="btn primary order-submit" type="submit"><MessageCircle/> فتح واتساب وإرسال الرسالة</button></form></div></div>}
